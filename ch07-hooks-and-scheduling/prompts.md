...
## Step 2 — Write the morning-brief script

> I want a shell script that runs every weekday at 6:30 AM and writes a
> "morning brief" file to ~/work/briefs/{date}.md. The brief should: read my
> unread Gmail (just the last 12 hours), pull today's calendar events, list any
> Notion mentions or assigned tasks, and write a short synthesis at the top —
> "Three things that matter today, in priority order, with one sentence each."
> Use my MCPs. Save the script to ~/work/scripts/morning-brief.sh. Make it
> idempotent — running it twice shouldn't double the output.
...
> Run morning-brief.sh now and show me the brief it produced.
...
## Step 5 — Add the SessionStart hook

> Add a SessionStart hook in ~/work/.claude/settings.json that reads today's
> brief from ~/work/briefs/{today}.md and shows it to me at the start of every
> session in this folder. If today's brief doesn't exist, say "no brief yet
> today" and don't crash.
...
> threads with each attendee, and any Notion docs they've recently edited. Show
> me the brief via a Stop hook on whatever I'm currently doing.
...
> ~/work/briefs/prs-pending-{date}.md, and post it as the first thing in my
> SessionStart hook output. If there are zero, say nothing.
...
> SessionStart hook should show it once on Sunday evening when I'm planning
> Monday. Keep the tone honest. If I missed something I said I'd do, name it. If
> a thing I worried about turned out to be nothing, name that too. Don't make me
> sound more productive than I was.
...
> about anything that moved more than 3%. Add it to SessionStart hook output for
> the next morning.
