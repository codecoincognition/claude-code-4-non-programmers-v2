...
description: >
  ...
  Calendar; cross-references against the Notion CRM database;
  proposes missing entries, stale stages, and cold rows that
  should be marked. NEVER writes to the CRM. Only writes proposals
...
3. Read the Notion CRM database. Note every row, its stage, last
   contact date, and any cold/dead status.
4. Cross-reference. Find three categories of mismatch:
   - Missing entry: counterparty present in outbox or calendar,
     not present in CRM.
   - Stale stage: CRM stage doesn't match the activity pattern
     (e.g., "Discovery" but Maya has been quoting prices weekly).
   - Cold without status: CRM row, last contact > 30 days ago,
...
- Write to the Notion CRM database. Not via any tool, ever.
